...
def train(opt):
    ...
    try:
        ...
        while True:
            ...
            optimizer.zero_grad()
            model_out = dp_lw_model(fc_feats, att_feats, labels, masks, att_masks, data['gts'], torch.arange(0, len(data['gts'])), sc_flag, use_arm=True)
            ...
            if True:
                print("iter {} (epoch {}), train_loss = {:.4f}, variance = {:g}, time/batch = {:.3f}" \
                    .format(iteration, epoch, train_loss, variance, end - start))
            else:
